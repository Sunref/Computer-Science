...
public class DAryMaxPriorityQueue<Key extends Comparable<Key>> implements PriorityQueue<Key> {
    ...
    private void swim(int k) {
        ...
        while (k > 1 && less(dAryParent(k), k)) {
            exchange(k, dAryParent(k)); // Troca com o pai
            k = dAryParent(k); // Atualiza o índice para o pai
        }
    }

    private void sink(int k) {
        
        while (dAryFirstChild(k) <= n) {
            int largest = dAryFirstChild(k); // Começa com o primeiro filho
            int lastChild = Math.min(n, dAryFirstChild(k) + d - 1); // Último filho
            for (int j = largest + 1; j <= lastChild; j++) {
                if (less(largest, j)) {
                    largest = j;
                }
            }
