...
public class TernaryMaxPriorityQueue<Key extends Comparable<Key>> implements PriorityQueue<Key> {
    ...
    private void swim(int k) {
        ...
        while (k > 1) {
            int parent = (k - 2) / 3 + 1; // Calcula o índice do pai fixando d = 3
            if (less(parent, k)) {
                exchange(k, parent); // Troca com o pai
                k = parent; // Atualiza o índice para o pai
            } else {
                break; // Se não houver violação, para
            }
        }
    ...
    private void sink(int k) {
        ...
        while (true) {
            int firstChild = 3 * (k - 1) + 2; // Calcula o índice do primeiro filho fixando d = 3
            if (firstChild > n) break; // Sai se não houver filhos

            int largest = firstChild; // Assume o maior como o primeiro filho
            for (int i = 1; i < 3; i++) { // Verifica os próximos dois filhos
                int child = firstChild + i;
                if (child <= n && less(largest, child)) {
                    largest = child; // Atualiza o maior
                }
            }

            if (!less(k, largest)) break; // Se o nó atual for maior ou igual ao maior filho, para

            exchange(k, largest); // Troca com o maior filho
            k = largest; // Atualiza o índice para o maior filho
        }
